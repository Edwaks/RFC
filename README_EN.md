...
**AiSDocs** is a library of canons and approved documents. It is designed for teams that require consistent decisions and terminology and makes current rules, decisions, and their relationships searchable.

- Public URL: https://rfc.aisaray.ru/
- Core entities: canons, documents, revisions, topics, links, statuses, and scopes.
- Key capabilities: document catalog, canon search, topic sections, document card, material relationships, revision status, library navigation.

## Why the Product Is Needed

A real workflow rarely consists of a single action. Users must understand the initial situation, gather information, choose the next step, coordinate a decision, and return to the result later. AiSDocs provides one domain-specific environment for this work. It preserves context between stages and shows not only an individual item, but also its place in the overall process.

The product’s main value is not the number of screens, but workflow continuity. Canons, documents, revisions, topics, links, statuses, and scopes are treated as parts of one user journey. New participants understand the current state faster, experienced specialists spend less time matching information manually, and managers gain a clear overview without requiring every step to be retold.
...
The roles Reader, Document author, and Canon owner work with the same domain model from different perspectives. Consistent names for objects and states reduce ambiguity during handoffs. A responsible participant can reference a specific object and expected result instead of repeating the entire context.

## Main Sections

The public description covers these user capabilities:

- **Document catalog** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Canon search** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Topic sections** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Document card** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Material relationships** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Revision status** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Library navigation** — a distinct part of the workflow, connected to the product model and a clear user outcome.
